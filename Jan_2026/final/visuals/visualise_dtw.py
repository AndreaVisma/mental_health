...
def dtw_distance_matrix_numba(sequences, cost_matrix, normalize):
    ...
    for i in prange(N):
        for j in range(i + 1, N):
            d = dtw_distance_numba(sequences[i], sequences[j], cost_matrix, normalize)
            dist[i, j] = d
            dist[j, i] = d
    return dist
...
################################
#### Young males
mini_df = seq_df[(seq_df.age_group_1 == "young 0-39") & (seq_df.sex_id == 1)]
...
def dtw_with_path(s1, s2, cost_matrix):
    """
    DTW with full cost matrix and optimal warping path.
    Returns:
    ...
    while i > 0 or j > 0:
        ...
        i, j = steps[np.argmin(costs)]

    path.reverse()
    return D[1:, 1:], path

def plot_dtw_matrix(D, path, seq1_labels, seq2_labels):
    plt.figure(figsize=(10, 8))
    plt.imshow(D, origin="lower", aspect="auto")
    plt.colorbar(label="Accumulated DTW cost")

    path_i, path_j = zip(*path)
    plt.plot(path_j, path_i, color="red", linewidth=2)

    plt.xticks(range(len(seq2_labels)), seq2_labels, rotation=90)
    plt.yticks(range(len(seq1_labels)), seq1_labels)

    plt.xlabel("Sequence 2")
    plt.ylabel("Sequence 1")
    plt.title("DTW Accumulated Cost Matrix with Optimal Path")
    plt.tight_layout()
    plt.show(block = True)

def plot_dtw_alignment(seq1_labels, seq2_labels, path):
    fig, ax = plt.subplots(figsize=(12, 4))

    y1, y2 = 1, 0

    for i, label in enumerate(seq1_labels):
        ax.text(i, y1, label, ha="center", va="center")
    for j, label in enumerate(seq2_labels):
        ax.text(j, y2, label, ha="center", va="center")

    for (i, j) in path:
        ax.plot([i, j], [y1, y2], alpha=0.4)

    ax.set_yticks([y2, y1])
    ax.set_yticklabels(["Sequence 2", "Sequence 1"])
    ax.set_xticks([])
    ax.set_title("DTW Alignment Diagram")
    ax.axis("off")
...
#####
# Example: pick two patients
row1 = mini_df.iloc[9]
row2 = mini_df.iloc[1]

seq1_blocks = ['F40-F48', 'F30-F39', 'E70-E90', 'I10-I15']
seq2_blocks = ['F40-F48', 'I20-I25', 'F30-F39', 'I30-I52']

# Map to indices
seq1_idx = [code_to_index[b] for b in seq1_blocks]
seq2_idx = [code_to_index[b] for b in seq2_blocks]

# DTW
D, path = dtw_with_path(seq1_idx, seq2_idx, cost_matrix)

# Visualisations
plot_dtw_matrix(D, path, seq1_blocks, seq2_blocks)
plot_dtw_alignment(seq1_blocks, seq2_blocks, path)

def apply_dtw_alignment(seq1, seq2, path):
    """
    Expand two sequences according to a DTW warping path.
    ...
    for i, j in path:
        ...
        aligned_2.append(seq2[j])

    return aligned_1, aligned_2

def block_to_y_mapping(blocks):
    unique_blocks = sorted(set(blocks))
    return {b: i for i, b in enumerate(unique_blocks)}

def plot_raw_trajectories(seq1, seq2, label1="Seq 1", label2="Seq 2"):
    all_blocks = seq1 + seq2
    y_map = block_to_y_mapping(all_blocks)

    x1 = range(len(seq1))
    y1 = [y_map[b] for b in seq1]

    x2 = range(len(seq2))
    y2 = [y_map[b] for b in seq2]

    plt.figure(figsize=(12, 4))
    plt.step(x1, y1, where="post", label=label1)
    plt.step(x2, y2, where="post", label=label2)

    plt.yticks(list(y_map.values()), list(y_map.keys()))
    plt.xlabel("Step")
    plt.ylabel("Diagnosis block")
    plt.title("Diagnosis trajectories (raw)")
    plt.legend()
    plt.tight_layout()
    plt.show(block = True)

def plot_aligned_trajectories(aligned_1, aligned_2, label1="Seq 1", label2="Seq 2"):
    all_blocks = aligned_1 + aligned_2
    y_map = block_to_y_mapping(all_blocks)

    x = range(len(aligned_1))
    y1 = [y_map[b] for b in aligned_1]
    y2 = [y_map[b] for b in aligned_2]

    plt.figure(figsize=(12, 4))
    plt.step(x, y1, where="post", label=label1)
    plt.step(x, y2, where="post", label=label2)

    plt.yticks(list(y_map.values()), list(y_map.keys()))
    plt.xlabel("DTW-aligned step")
    plt.ylabel("Diagnosis block")
    plt.title("Diagnosis trajectories (DTW aligned)")
    plt.legend()
    plt.tight_layout()
    plt.show(block = True)

seq1 = ['F40-F48', 'F30-F39', 'E70-E90', 'I10-I15']
seq2 = ['F30-F39', 'F40-F48', 'I20-I25', 'F80-F89', 'I30-I52']

# Map to indices
seq1_idx = [code_to_index[b] for b in seq1]
seq2_idx = [code_to_index[b] for b in seq2]

# DTW
D, path = dtw_with_path(seq1_idx, seq2_idx, cost_matrix)

# Apply alignment
aligned_1, aligned_2 = apply_dtw_alignment(seq1, seq2, path)

# Plot
plot_raw_trajectories(seq1, seq2, label1="Patient A", label2="Patient B")
plot_aligned_trajectories(aligned_1, aligned_2, label1="Patient A", label2="Patient B")
