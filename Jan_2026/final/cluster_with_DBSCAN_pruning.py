...
top_12_nats = seq_df.country.value_counts().reset_index().country.iloc[1:13].tolist()
...
mini_df = seq_df[(seq_df.age_group_1 == age_group) & (seq_df.sex_id == sex_id)].copy().reset_index(drop = True)
subsample = mini_df.copy()
ids = subsample["patient_no"].tolist()

#######################
#
# def knn_distance_histogram(D, k, bins=50):
#     # Copy so we don't destroy original matrix
#     D_work = D.copy()
#
...
#
#     # Sort each row
#     sorted_D = np.sort(D_work, axis=1)
#
#     # k-th nearest neighbor distance (k starts at 1)
#     knn_distances = sorted_D[:, k-1]
#
#     # Plot
#     plt.hist(knn_distances, bins=bins)
#     plt.xlabel(f"{k}-NN distance")
#     plt.ylabel("Count")
#     plt.title(f"Histogram of {k}-NN distances")
#     plt.show(block = True)
#
#     return knn_distances
#
# d_small = d[:10_000, :10_000]
# d_small_old = d_old[:1_000, :1_000]
#
# knn_distance_histogram(d_small, 10)
#
# for k in [1, 10, 20]:
#     knn_distance_histogram(d_small, k)
#
# def knn_distance_ratio(D, k1=1, k2=20):
#     D_work = D.copy()
#     D_work[D_work == 0] = 0.001
#     np.fill_diagonal(D_work, np.inf)
#     sorted_D = np.sort(D_work, axis=1)
#
#     return sorted_D[:, k2-1] / sorted_D[:, k1-1]
#
# ratios = knn_distance_ratio(d_small, k1=1, k2=20)
# np.percentile(ratios, [5, 25, 50, 75, 90,95])

################
# first tune dbscan on a smaller sample
d_small = d[:10_000, :10_000].copy()

d_filled = d_small.copy()
np.fill_diagonal(d_filled, np.inf)
d_k = np.sort(d_filled, axis=1)[:, 4]  # 5th NN

ys = np.percentile(d_k, [x for x in range(1,100)])
plt.plot(ys)
plt.show(block = True)

shares_noise = []
for e in tqdm([0.15, 0.2, 0.25, 0.3, 0.35]):
    db = DBSCAN(metric = "precomputed", eps = e, min_samples = 5)
    db.fit(d_small)
    shares_noise.append(np.where(db.labels_ == -1, True, False).mean())

db = DBSCAN(metric = "precomputed", eps = 0.2, min_samples = 5)
db.fit(d_small)
shares_noise.append(np.where(db.labels_ == -1, True, False).mean())
core_mask = db.labels_ != -1
core_indices = np.where(core_mask)[0]

d_core = d_small[np.ix_(core_indices, core_indices)]
...
for k in tqdm(k_values):
    ...
    labels = kmedoids.fit_predict(d_core)
...
# ax1.set_yscale('log')
ax.set_title("Elbow Method (K-Medoids)")
ax.set_xlabel("k")
ax.set_ylabel("Total Distance to Medoids")

plt.tight_layout()
fig.savefig(outfile + f"{sex_dict[sex_id]}_{age_group}//elbow.png")
plt.show(block=True)

##################
#prune based on dbscan
db = DBSCAN(metric = "precomputed", eps = 0.2, min_samples = 5)
db.fit(d)
...
d_core = d[np.ix_(core_indices, core_indices)].copy()
mini_df = mini_df.reset_index(drop = True).iloc[core_indices]
...
#### demographics
df_dem = mini_df[["patient_no", "cluster", "sequence_unique_blocks", "is_foreign", "age", "ag_id", "country"]].copy()
...
if age_group.startswith("y"):
    bins = np.linspace(0, 39, 9)
elif age_group.startswith("m"):
    bins = np.linspace(39, 64, 6)
else:
    bins = np.linspace(64, 94, 7)
...
table.to_excel(outfile + f"{sex_dict[sex_id]}_{age_group}//{sex_dict[sex_id]}_{age_group}_clusters_means_summary.xlsx", index = False)
...
summary_df.to_excel(outfile + f"{sex_dict[sex_id]}_{age_group}//{sex_dict[sex_id]}_{age_group}_top_diagnoses_summary.xlsx", index = False)
...
df_un_traj.to_excel(outfile + f"{sex_dict[sex_id]}_{age_group}//{sex_dict[sex_id]}_{age_group}_nr_unique_trajectories.xlsx", index = False)
mct_all.to_excel(outfile + f"{sex_dict[sex_id]}_{age_group}//{sex_dict[sex_id]}_{age_group}_top_10_most_common_trajectories.xlsx", index = False)
mct_all.to_csv(outfile + f"{sex_dict[sex_id]}_{age_group}//{sex_dict[sex_id]}_{age_group}_top_10_most_common_trajectories.csv", index = False)
########
